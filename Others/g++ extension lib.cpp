...
/**
 * 1) push(x) 将 x 插入堆中，并返回这个元素的迭代器
 * 2) modify(it, x) 修改迭代器 it 指向的元素为 x
 * 3) erase(it) 删除迭代器 it 指向的元素
 * 4) erase_if(p) 删除满足谓词 p 的元素
 * 5）split(p, t) 将满足谓词 p 的元素分裂到堆 t 中
 * 6）join(t) 将堆 t 中的元素移动到当前堆中
 */